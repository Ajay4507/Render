...
def connect_to_database():
    try:
        con = psycopg2.connect(
            host='dpg-cog0rif79t8c73cd8i80-a.oregon-postgres.render.com',
            user='pythonflask',
            password='CD4do7YIFKg9DnWJThju3LVTLnwA9Ym6',
...
def load_data():
    ...
    if not con:
        return jsonify({'error': {'category': 'IO Error', 'type': 'database_connection_error', 'message': 'Failed to load data. Unable to connect to the database.'}})
    ...
    try:
        ...
        for item in data:
            ...
            if not organization.is_valid():
                ...
                error_message = "Organization with ID {} is missing required fields: {}".format(organization.id, ', '.join(not_filled_fields))
                return jsonify({'error': {'category': 'Value Error', 'type': 'data_validation_error', 'message': error_message}})

            val = (organization.id, organization.name, organization.type, organization.created_by, organization.creation_date, organization.updated_on, organization.updated_by)
